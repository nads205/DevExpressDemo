...
<!-- 
	The Qatar Program's MARC-to-MODS stylesheet
	version 1.21, 15.03.2013	
	
	This style sheet is based on the official MARC21slin2MODS3.5 stylesheet; revision 1.84
	version 1.21:
	- uses modification date from "LAS" control field
	
	version 1.2:
	- removed "xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
			  xsi:schemaLocation="http://www.loc.gov/mods/v3          
			  http://www.loc.gov/standards/mods/v3/mods-3-4.xsd"
	  this will be part of the METS header anyhow
	
	- considers approximate dates in controlfield 008
	- consider unknown dates in controlfield 008 ('||||')
	- doesn't consider MARC 852 fields as holding data (location) isn't considered to be relevant
	
	version 1.1:
	- added maps coordinates and conversions from degress to decimal
	- 
...
-->
		...
		<mods:originInfo>
			...
			<!-- me: version 1.2 dealing with the YYYu and YYuu cases where u expresses
			         unknown dates -->
			<xsl:variable name="controlField008-7-8"
				select="normalize-space(substring($controlField008, 8, 2))"/>
			<xsl:variable name="controlField008-7-9"
				select="normalize-space(substring($controlField008, 8, 3))"/>
			<xsl:variable name="controlField008-9-10"
				select="normalize-space(substring($controlField008, 10, 1))"/>
			<xsl:variable name="controlField008-10-11"
				select="normalize-space(substring($controlField008, 11, 1))"/>
			...
			<xsl:if
				...
				<xsl:if test="$controlField008-7-10 and ($controlField008-7-10 != $dataField260c) and $controlField008-7-10!='||||'">
			...
			<xsl:if
				...
				<xsl:if test="$controlField008-7-10 and ($controlField008-7-10 != $dataField260c)">
					<!-- only if the exact date/year is known -->
					<xsl:if test="$controlField008-7-10 and $controlField008-9-10!='u' and $controlField008-10-11!='u' and $controlField008-7-10!='||||'">				
			...
			<xsl:if
				...
				<xsl:if test="$controlField008-7-10 and $controlField008-7-10!='||||'">
					<mods:dateIssued encoding="marc" point="start">
						<xsl:value-of select="$controlField008-7-10"/>
					</mods:dateIssued>
				</xsl:if>
			...
			<xsl:if test="$controlField008-6='s'">
				<xsl:if test="$controlField008-7-10 and $controlField008-9-10='u' and $controlField008-10-11='u' and $controlField008-7-10!='||||'">
					<!-- the two last ones are u -->
					<mods:dateIssued encoding="marc" qualifier="approximate" point="start">
						<xsl:value-of select="$controlField008-7-8"/>00</mods:dateIssued>
					<mods:dateIssued encoding="marc" qualifier="approximate" point="end">
						<xsl:value-of select="$controlField008-7-8"/>99</mods:dateIssued>
				</xsl:if>	
				<xsl:if test="$controlField008-7-10 and $controlField008-9-10!='u' and $controlField008-10-11='u' and $controlField008-7-10!='||||'">
					<!-- the last one is a u -->
					<mods:dateIssued encoding="marc" qualifier="approximate" point="start">
						<xsl:value-of select="$controlField008-7-9"/>0</mods:dateIssued>
					<mods:dateIssued encoding="marc" qualifier="approximate" point="end">
						<xsl:value-of select="$controlField008-7-9"/>9</mods:dateIssued>
				</xsl:if>
				<xsl:if test="$controlField008-7-10 and $controlField008-9-10!='u' and $controlField008-10-11!='u' and $controlField008-7-10!='||||'">
					<!-- no u-->
					<mods:dateIssued encoding="marc">
		...
		<xsl:for-each select="marc:datafield[@tag=264][@ind2=0]">
			<mods:originInfo displayLabel="producer">
		...
		<xsl:for-each select="marc:datafield[@tag=264][@ind2=1]">
			<mods:originInfo displayLabel="publisher">
				<!-- Template checks for altRepGroup - 880 $6 -->
				<xsl:call-template name="xxx880"/>
				<mods:place>
					<mods:placeTerm>
		...
		<xsl:for-each select="marc:datafield[@tag=264][@ind2=2]">
			<mods:originInfo displayLabel="distributor">
				<!-- Template checks for altRepGroup - 880 $6 -->
				<xsl:call-template name="xxx880"/>
				<mods:place>
					<mods:placeTerm>
		...
		<xsl:for-each select="marc:datafield[@tag=264][@ind2=3]">
			<mods:originInfo displayLabel="manufacturer">
				<!-- Template checks for altRepGroup - 880 $6 -->
				<xsl:call-template name="xxx880"/>
				<mods:place>
					<mods:placeTerm>
...
-->
		...
		<!--	location	-->
		<!-- Don't use 852; will be for holdings only -->
		<!--
		...
		<mods:recordInfo>
			...
			<!-- -->
			<xsl:for-each select="marc:datafield[@tag='LAS']">
				
				<xsl:variable name="changedate" select="marc:subfield[@code='c']"/>
				<xsl:variable name="changetime" select="marc:subfield[@code='h']"/>
				<xsl:variable name="changeyear" select="substring($changedate,1,4)"/>
				<xsl:variable name="changemonth" select="substring($changedate,5,2)"/>
				<xsl:variable name="changeday" select="substring($changedate,7,2)"/>
				<xsl:variable name="changehour" select="substring($changetime,1,2)"/>
				<xsl:variable name="changeminute" select="substring($changetime,3,2)"/>
				
				<mods:recordChangeDate encoding="iso8601">
					<xsl:value-of select="$changeyear"/>-<xsl:value-of select="$changemonth"/>-<xsl:value-of
						select="$changeday"/>T<xsl:value-of select="$changehour"/>:<xsl:value-of select="$changeminute"/>:00</mods:recordChangeDate>
			</xsl:for-each>
			...
			<mods:recordOrigin>Converted from MARCXML to MODS version 3.4 using MARC2MODS_XSLT stylesheets for maps; version 15.03.2013 - evening</mods:recordOrigin>
...
-->
	...
	<!-- geoRSS data in mods:extension 
		latitude / longitude
		
		information is converted from degrees/minutes/seconds
		to decimal degrees
	-->
    <xsl:template name="georss">
    	
    	
    	<xsl:if test="//marc:datafield[@tag='034']/marc:subfield[@code='d']">
    		<mods:extension>
    			<georss:box>
    				<xsl:call-template name="georsslonglat">
    					<xsl:with-param name="str" select="//marc:datafield[@tag='034']/marc:subfield[@code='f']"/>
    				</xsl:call-template>
    				<xsl:text> </xsl:text>
    				
    				<xsl:call-template name="georsslonglat">
    					<xsl:with-param name="str" select="//marc:datafield[@tag='034']/marc:subfield[@code='d']"/>
    				</xsl:call-template>
    				<xsl:text> </xsl:text>
    				
    				<xsl:call-template name="georsslonglat">
    					<xsl:with-param name="str" select="//marc:datafield[@tag='034']/marc:subfield[@code='g']"/>
    				</xsl:call-template>
    				<xsl:text> </xsl:text>
    				
    				<xsl:call-template name="georsslonglat">
    					<xsl:with-param name="str" select="//marc:datafield[@tag='034']/marc:subfield[@code='e']"/>
    				</xsl:call-template>
	...
	<!-- converts MARC into long/lat coordinates with + and - -->

	<xsl:template name="georsslonglat">	
		<xsl:param name="str"/>	
		...
		<xsl:if test="$str">
			...
			<xsl:variable name="first-char" select="substring($str,1,1)"/>
			<xsl:choose>
				<xsl:when test="contains('N',$first-char)">
					<xsl:text>+</xsl:text>
				</xsl:when>
				<xsl:when test="contains('S',$first-char)">
					<xsl:text>-</xsl:text>
				</xsl:when>
				<xsl:when test="contains('E',$first-char)">
					<xsl:text>+</xsl:text>
				</xsl:when>
				<xsl:when test="contains('W',$first-char)">
					<xsl:text>-</xsl:text>
				</xsl:when>
			</xsl:choose>
			<xsl:call-template name="todecimaldegrees">
				<xsl:with-param name="degrees">
					<xsl:value-of select="substring($str,2,3)"/>
				</xsl:with-param>
	...
	<xsl:template name="todecimaldegrees">
		<xsl:param name="degrees"/>
		<xsl:param name="minutes"/>
		<xsl:param name="seconds"/>
		
		<xsl:variable name="a" select="$degrees div 1"/>
		<xsl:variable name="b" select="$minutes div 60"/>
		<xsl:variable name="c" select="$seconds div 3600"/>
		<xsl:value-of select="substring($a + $b + $c,1,10)" />
	</xsl:template>
